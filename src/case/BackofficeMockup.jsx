...
import { TapPulse, SuccessCheck } from '@/case/StoryDevices'

// Desktop-window mockup of the wholesaler back-office. Brand-neutral (Kalza
// base). `phase`: undefined = static · 1 = tapping confirm · 2+ = confirmed.
export function BackofficeMockup({ className, phase }) {
  const { t } = useLang()
  const done = phase >= 2
  return (
      ...
      <div className="relative flex">
        ...
        <div className="flex-1 p-3.5">
          ...
          <div className="mb-3 rounded-lg border p-3">
            <div className="flex items-center justify-between">
              ...
              >
                {done ? t.phone.steps[1] : t.phone.steps[0]}
              </span>
            </div>
        ...
        {done && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 bg-background/85 backdrop-blur-sm">
            <SuccessCheck />
            <span className="text-sm font-semibold text-success">{t.phone.steps[1]}</span>
          </div>
        )}
      </div>
    </div>
